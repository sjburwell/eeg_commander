...
config(3).userinput =['tmpEEG = EEG; ' ...
                      'tmpEEG.ref = ''common''; ' ...
                      'tmpEEG = iclabel(tmpEEG); ' ...
                      '[~,idx]= max(tmpEEG.etc.ic_classification.ICLabel.classifications''); ' ...
                      'if ~isempty(find(idx==3)), EEG = pop_subcomp(EEG,find(idx==3),0); ' ...
                      'else, pop_topoplot(EEG,0,1:size(EEG.icawinv,2)); pop_eegplot(EEG,1,0,0); pop_eegplot(EEG,0,0,0); ' ...
                      'EEG = pop_subcomp(EEG); close all; ' ...
                      'end; '];
...
% ];
config(4).userinput = 'EEG = pop_firws(EEG, ''fcutoff'', 55, ''ftype'', ''lowpass'', ''wtype'', ''kaiser'', ''warg'', 7.85726, ''forder'',88);';
