...
%    if length(args.swap_elecs) == 1; args.swap_elecs{2} = 'Unused'; end %, changed 6/14/13 b/c Unused_f in some files, *pin *flat
    if length(args.swap_elecs) == 1; args.swap_elecs{2} = 'Unused_p'; end
...
%Event values, check and if non-existent, try to find!
%if isempty(EEG.event), 
   %uEvents = zeros(EEG.nbchan,1); 
   %for i = 1:EEG.nbchan, uEvents(i) = length(unique(EEG.data(i,:))); end
