...
%                 -- for complex distribtions only (e.g. RID Rihajcek and complex morlet wavelets)  
%                    See README_loadvars_time-frequency.txt for detials, 
%                       -Where SETvars.TFDparams.TFPS becomes OPTIONS.TFPS.  
%                       -MUST use appropriate complex TF transform in domain parameter above 
% 
...
% vars
  ...
  %% bberp.catcodes    = zeros(tempsizerows,1); %always out
  %if isstruct(catcodes2extract) && isfield(catcodes2extract,'name') && ischar(catcodes2extract(1).name),
  %   berp.catcodes = ''; 
  %else,
  %   berp.catcodes = []; 
  %end
  berp.catcodes = ''; %scott changed, as opposed to above 4 lines 
...
for main_loop_counter=1:length(subnames(:,1)),
  ...
  %pack % not compatible with matlab7 
   ...
   %  end
   %end %% replaced by below to handle cellstring inputs of varying availability
  
   % generate catcodes if not given (default ALL ttypes in file)
   if create_catcodes>=1,
     clear cats ttypes catcodes catcodes2extract;
     if isfield(erp,'stimkeys')&&isfield(erp.stimkeys,'ttype')&&~isempty(erp.stimkeys.ttype),
        cats = erp.stimkeys.ttype;
     else, 
        cats = strtrim(cellstr(num2str(unique(erp.ttype)))');
     end
     for j=1:length(cats),
         eval(['catcodes2extract(' num2str(j) ').name = ''' cats{j} ''';' ]);
         eval(['catcodes2extract(' num2str(j) ').text = ''erp.ttype==' num2str(j) ''';' ]);
     end
   end
     ...
     % define TFPS reference elec data 
       ...
       if isfield(domainparms,'TFPS') && ~isequal(domainparms.TFPS,0), 
         if isequal(domainparms.TFPS.method, 'interelec'),  
           ...
           cur_vect_ref     = erp.elec==strmatch(domainparms.TFPS.refelec,erp.elecnames,'exact')&cur_catcode_vect==1&reject.trials2reject==0; 
     ...
     % loop electrodes 
       for e = 1:length(elecnums2extract), 
          cur_elec = elecnums2extract(e);
            ...
            cur_vect     = erp.elec==cur_elec&cur_catcode_vect==1&reject.trials2reject==0; 
...
%                      rmfield(erp.stim,'extract_temp');
%                      temperp_ref.data = temperp_ref.data(:,startbin:endbin); 
%                      temperp_big(1,:) = temperp; 
%                      temperp_big(2,:) = temperp_ref; 
...
%                    if isequal(domainparms.TFPS.method,'intertrial'), 
%                      c = zeros(1,tempsizefreq,tempsizetime); 
%                    elseif isequal(domainparms.TFPS.method,'interelec'),  
%                      c = zeros(1,tempsizefreq,tempsizetime);
%                     %c = temperp.data; 
%                    end 
                   else, 
